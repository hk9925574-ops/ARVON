...
export class SportsKnowledgeProvider implements IKnowledgeProvider {
  name = 'Live Sports Knowledge';
  ...
  private readonly CACHE_TTL_MS = 2 * 60 * 1000; // 2 minutes for sports
  ...
  async retrieve(text: string, context?: any): Promise<KnowledgeResult[]> {
    ...
    try {
      ...
      const response = await fetch(url, {
        ...
        body: `q=${encodeURIComponent(optimizedQuery + ' score match result')}`
      });
      ...
      const topResults = ranked.slice(0, 3).map(r => ({
          source: KnowledgeSource.WEB,
          content: `[SPORTS] ${r.title}\n${r.snippet}\nSource: ${r.url}`,
          confidence: 0.9,
      ...
      console.error('[ARVON][SportsKnowledge] Error:', e);
      return [];
    }
  }
}
